...
const searchSettings = {
    ...
    save() {
        localStorage.setItem('hranilishe_search_settings', JSON.stringify(this));
    },
    // Для загрузки из localStorage
    load() {
        const saved = localStorage.getItem('hranilishe_search_settings');
        if (saved) {
...
function initSearchSettings() {
    ...
    const settingsModal = document.getElementById('search-settings-modal');
    ...
    resetBtn.addEventListener('click', () => {
        ...
        updateActiveFilters();
        // Выполняем поиск с новыми настройками
...
function updateSettingsUI() {
    // Поля поиска
    document.getElementById('search-title').checked = searchSettings.fields.title;
    document.getElementById('search-description').checked = searchSettings.fields.description;
    document.getElementById('search-short-description').checked = searchSettings.fields.shortDescription;
    document.getElementById('search-tags').checked = searchSettings.fields.tags;
    document.getElementById('search-author').checked = searchSettings.fields.author;
    
    // Фильтр по автору
    document.querySelector(`input[name="author-filter"][value="${searchSettings.authorFilter}"]`).checked = true;
    
    // Диапазон дат
    document.getElementById('date-from').value = searchSettings.dateRange.from || '';
    document.getElementById('date-to').value = searchSettings.dateRange.to || '';
    
    // Сортировка
    document.querySelector(`input[name="sort"][value="${searchSettings.sortBy}"]`).checked = true;
...
function saveSettingsFromUI() {
    // Поля поиска
    searchSettings.fields.title = document.getElementById('search-title').checked;
    searchSettings.fields.description = document.getElementById('search-description').checked;
    searchSettings.fields.shortDescription = document.getElementById('search-short-description').checked;
    searchSettings.fields.tags = document.getElementById('search-tags').checked;
    searchSettings.fields.author = document.getElementById('search-author').checked;
    
    // Фильтр по автору
    searchSettings.authorFilter = document.querySelector('input[name="author-filter"]:checked').value;
    
    // Диапазон дат
    searchSettings.dateRange.from = document.getElementById('date-from').value || null;
    searchSettings.dateRange.to = document.getElementById('date-to').value || null;
    
    // Сортировка
    searchSettings.sortBy = document.querySelector('input[name="sort"]:checked').value;
    
    // Сохраняем
    searchSettings.save();
}

// Инициализация фильтра тегов
function initTagsFilter() {
    ...
    allTags.forEach(tag => {
        ...
        if (searchSettings.selectedTags.includes(tag)) {
            tagElement.classList.add('selected');
        }
        
        tagElement.innerHTML = `
            <input type="checkbox" ${searchSettings.selectedTags.includes(tag) ? 'checked' : ''}>
            <span>${tag}</span>
        `;
        ...
        tagElement.addEventListener('click', (e) => {
            ...
            if (checkbox.checked && !searchSettings.selectedTags.includes(tag)) {
                searchSettings.selectedTags.push(tag);
            } else if (!checkbox.checked) {
                const index = searchSettings.selectedTags.indexOf(tag);
                if (index > -1) {
                    searchSettings.selectedTags.splice(index, 1);
                }
            }
            
            // Ограничиваем 5 тегами
            if (searchSettings.selectedTags.length > 5) {
                const oldestTag = searchSettings.selectedTags.shift();
                const oldestElement = document.querySelector(`.tag-option span:contains('${oldestTag}')`)?.parentElement;
                if (oldestElement) {
                    oldestElement.classList.remove('selected');
                    oldestElement.querySelector('input').checked = false;
                }
            }
...
// Обновление UI фильтра тегов
function updateTagsFilterUI() {
    ...
    allTagOptions.forEach(option => {
        ...
        if (searchSettings.selectedTags.includes(tagName)) {
...
function updateActiveFilters() {
    ...
    // Поля поиска
    const disabledFields = Object.entries(searchSettings.fields)
    ...
    disabledFields.forEach(field => {
        ...
        addActiveFilter(`${fieldNames[field]} выкл.`, 'field', field);
    });
    
    // Фильтр по автору
    if (searchSettings.authorFilter !== 'all') {
        const authorFilterNames = {
            verified: 'Проверенные авторы',
            anonymous: 'Анонимы',
            teams: 'Команды'
        };
        addActiveFilter(authorFilterNames[searchSettings.authorFilter], 'author', searchSettings.authorFilter);
    }
    
    // Диапазон дат
    if (searchSettings.dateRange.from) {
        addActiveFilter(`От ${searchSettings.dateRange.from}`, 'date-from', searchSettings.dateRange.from);
    }
    if (searchSettings.dateRange.to) {
        addActiveFilter(`До ${searchSettings.dateRange.to}`, 'date-to', searchSettings.dateRange.to);
    }
    
    // Выбранные теги
    searchSettings.selectedTags.forEach(tag => {
        addActiveFilter(tag, 'tag', tag);
    });
    
    // Сортировка
    if (searchSettings.sortBy !== 'relevance') {
        const sortNames = {
            'date-desc': 'Новые → старые',
            'date-asc': 'Старые → новые',
            'title-asc': 'А-Я',
            'title-desc': 'Я-А'
        };
        addActiveFilter(sortNames[searchSettings.sortBy], 'sort', searchSettings.sortBy);
    }
}

// Добавление активного фильтра
...
function removeFilter(type, value) {
    switch (type) {
        case 'field':
            searchSettings.fields[value] = true;
            break;
        case 'author':
            searchSettings.authorFilter = 'all';
            break;
        case 'date-from':
            searchSettings.dateRange.from = null;
            break;
        case 'date-to':
            searchSettings.dateRange.to = null;
            break;
        case 'tag':
            searchSettings.selectedTags = searchSettings.selectedTags.filter(tag => tag !== value);
            break;
        case 'sort':
...
function performAdvancedSearch(query = '') {
    const searchInput = document.querySelector('.search-input');
    const currentQuery = query || searchInput?.value || '';
    
    let filteredItems = items;
    
    // 1. Поиск по тексту (если есть запрос)
    if (currentQuery.trim()) {
        const lowerQuery = currentQuery.toLowerCase();
        filteredItems = items.filter(item => {
            const searchIn = [];
            
            if (searchSettings.fields.title) searchIn.push(item.title);
            if (searchSettings.fields.description) searchIn.push(item.description);
            if (searchSettings.fields.shortDescription) searchIn.push(item.shortDescription);
            if (searchSettings.fields.tags) searchIn.push(...item.tags);
            if (searchSettings.fields.author && item.author) searchIn.push(item.author);
            
            return searchIn.some(text => 
                String(text).toLowerCase().includes(lowerQuery)
            );
        });
    }
    ...
    if (searchSettings.authorFilter !== 'all') {
        filteredItems = filteredItems.filter(item => {
            ...
            switch (searchSettings.authorFilter) {
                case 'verified':
                    return hasAuthor;
                case 'anonymous':
                    return !hasAuthor || item.author === 'Аноним' || item.author === 'Неизвестно';
                case 'teams':
                    return hasAuthor && (
                        item.author.includes('Команда') || 
                        item.author.includes('Cuby') ||
                        item.author.includes('TAIPrompts') ||
                        item.author.includes('ICEBERG')
                    );
                default:
                    return true;
            }
    ...
    // 3. Фильтр по дате (нужно добавить поле date во все items!)
    if (searchSettings.dateRange.from || searchSettings.dateRange.to) {
        filteredItems = filteredItems.filter(item => {
            const itemDate = parseDate(item.date);
            if (!itemDate) return true; // Если даты нет - пропускаем
            
            if (searchSettings.dateRange.from) {
                const fromDate = new Date(searchSettings.dateRange.from);
                if (itemDate < fromDate) return false;
            }
            
            if (searchSettings.dateRange.to) {
                const toDate = new Date(searchSettings.dateRange.to);
    ...
    // 4. Фильтр по тегам
    if (searchSettings.selectedTags.length > 0) {
        filteredItems = filteredItems.filter(item => 
            searchSettings.selectedTags.some(tag => 
    ...
    // 5. Сортировка
    filteredItems = sortItems(filteredItems, searchSettings.sortBy);
    
    // Отображение результатов
    renderSearchResults(filteredItems, currentQuery);
}
...
function sortItems(itemsToSort, sortBy) {
    ...
    switch (sortBy) {
        ...
        case 'relevance':
        default:
            // Для релевантности - сначала по совпадению в заголовке, потом по дате
            return sorted.sort((a, b) => {
                // Можно добавить логику релевантности по поисковому запросу
...
function parseDate(dateString) {
    if (!dateString || dateString === 'Неизвестно') return null;
...
function renderSearchResults(filteredItems, query) {
    const allItemsContainer = document.getElementById('all-items');
    const paginationContainer = document.getElementById('pagination');
    const featuredItemsContainer = document.getElementById('featured-items');
    
    if (!allItemsContainer) return;
    ...
    if (filteredItems.length === 0) {
        allItemsContainer.innerHTML = `
            <div class="no-items">
                ...
                <p>${query ? `По запросу "${query}" ничего не найдено` : 'По вашим фильтрам ничего не найдено'}</p>
                <button class="btn-secondary" onclick="resetSearch()">Сбросить фильтры</button>
            </div>
        `;
        paginationContainer.innerHTML = '';
        return;
    ...
    if (query.trim() || Object.keys(searchSettings).some(key => {
        if (key === 'fields') return Object.values(searchSettings.fields).some(v => !v);
        if (key === 'authorFilter') return searchSettings.authorFilter !== 'all';
        if (key === 'dateRange') return searchSettings.dateRange.from || searchSettings.dateRange.to;
        if (key === 'selectedTags') return searchSettings.selectedTags.length > 0;
        if (key === 'sortBy') return searchSettings.sortBy !== 'relevance';
        return false;
    })) {
        paginationContainer.innerHTML = '';
    } else {
...
// Обновляем существующие обработчики поиска
function updateSearchHandlers() {
    const searchInputs = document.querySelectorAll('.search-input');
    const searchBtns = document.querySelectorAll('.search-btn');
    
    // Обновляем обработчики для кнопки поиска
    ...
    // Обновляем обработчики для Enter в поле поиска
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем контейнер для активных фильтров
